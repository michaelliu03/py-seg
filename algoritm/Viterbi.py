...
class Viterbi:
    """
      ...
      """
    @staticmethod
    def computeEnumSimply(roleTagList,transformMatrixDictionay,tag):
        """
              仅仅利用了转移矩阵的“维特比”算法
        ...
        for i in range(length):
            ...
            item = iterator.next()
            for cur in item:
                now = transformMatrixDictionay.transititon_probability[tag.ordinal(pre[0])][
                          tag.ordinal(cur[0])] - math.log(
                    (cur[1] + 1e-8) / transformMatrixDictionay.getTotalFrequency(cur[0]))
                if perfect_cost > now:
                    perfect_cost = now
                    perfect_tag = cur
            pre = perfect_tag
