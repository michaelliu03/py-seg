...
class ViterbiSegment(WordBasedGenerativeModelSegment,metaclass=ABCMeta):
        ...
        for i in range(1, len(nodes) - 1):
            ...
            for index in range(len(nodeArray)):
                ...
                if node.fromnode is None:
                    continue
                for ind in range(len(nodes[i + len(node.realword)])):
                    to = nodes[i + len(node.realword)][ind]
                    to.updateFrom(node)
